...
RUN GOPATH=`go env GOPATH` && PATH=$PATH:$GOPATH/bin && cd /tmp && GO111MODULE=off go get -d -u github.com/golang/protobuf/protoc-gen-go && git -C "$(go env GOPATH)"/src/github.com/golang/protobuf checkout "v1.3.2" && go install github.com/golang/protobuf/protoc-gen-go
